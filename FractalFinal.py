...
class FractalDimention:
        ...
        def PlotPoints(self,LogDir,starttime,endtime):
            ...
            fileio.write(str("\n Dimentions of the image = ")+str(self.BinaryImageArray.shape[0])+" x "+str(self.BinaryImageArray.shape[1]) )
            #fileio.write(str("\nParameters = ")+str(parg))
            #fileio.write(str("\nCovariance Matrix = ")+str(popt))
            fileio.write(str("\n Total Time elapsed :: ")+str(endtime-starttime)+" units")
            fileio.write(str("\n Fractal Dimention of Image (D)= "+str(slope)))
            fileio.write("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")

            #Plot the best fit line
            mp.plot(x0, intercept + x0*slope, 'b-', label = 'Best Fit Line')

            #Add labels to the image
            mp.grid(True)
            mp.legend()
            mp.title(self.imgName+'.png')
            mp.xlabel(str(" log 1/si ----> ( Log Base 10 ) Fractal Dimention of Image = ")+str(slope))
            mp.ylabel(" log ni ----> ( Log Base 10 )")
            mp.savefig(LogDir+"Plot-"+imgName+".png",facecolor='w', edgecolor='w',
